...
test("English has no prefix and sets <html lang>", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 1 })).toHaveText("Ledger Flow");
  await expect(page.locator("main p")).toContainText("three seconds");
});

test("Spanish lives under /es", async ({ page }) => {
  await page.goto("/es");
  await expect(page.locator("html")).toHaveAttribute("lang", "es");
  await expect(page.locator("main p")).toContainText("tres segundos");
});
