...
export async function PublicFrame({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-bg pb-(--safe-bottom) text-text">
      <header className="mx-auto flex w-full max-w-(--content-max) items-center gap-2 px-4 py-4 sm:gap-3 sm:px-6 md:px-8">
        ...
        <Link
          href={path}
          locale={otherLocale}
          aria-label={`${t("nav.language")}: ${otherLocale.toUpperCase()}`}
          className="ms-auto inline-flex h-8 items-center gap-1.5 rounded-full border border-border-strong px-3 text-sm font-medium uppercase"
        >
          <Globe {...iconProps("sm")} />
          {otherLocale}
        </Link>
