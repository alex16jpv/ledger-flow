...
- **What** — a radio group of two rows, each with a visible radio mark and **its own count**. _What
  ...
  list's count ignores the search, so **with a search in force the device counts the matches first**, away from the screen like the file itself,
...
- **The copy is still arriving** (`#export-no-copy`), the account's first download running: "This
  ...
  wakes up by itself the moment the copy can answer.
- **The first download keeps failing** (`#export-no-copy-failing`), online but a server error or a dead
  session: "This device couldn't get your transactions from the server, so there is nothing to
  download from yet." with `Try again`, which asks for the download at once.
- **No copy and no connection** (`#export-no-copy-offline`): "Needs a connection first: this device
  doesn't have your transactions yet."
- **The copy could not be opened** (`#export-no-copy-broken`), storage that exists but refused (a
  blocked upgrade, a full disk): "This device couldn't open its copy of your data. Reloading the app
  usually fixes it." with `Reload`.
...
A copy that has everything but the time zone it cuts days with (an old copy, H-14) answers _All
transactions_ and not a period, so only the period row is off, with the first state's sentence. In
_this device only_ the copy is all there is, and it is always what the file is made from.

**Preparing** (`#export-preparing`): the file is written away from the screen, in batches, so the app
never freezes however long the history is. It is not a snapshot: if the copy is reset while it runs (a full resync, a sign-out), the build fails rather than hand over a file with half the rows. The primary button spins in place, keeping the name
...
block or cancel, and the app cannot know. On an iPhone's installed app, where a download opens a viewer, the toast reads "Your file is ready"
with `Share file` (`#export-done-share`): the share sheet needs a tap of its own, and a long build has
used up the one that started it. **If building it fails**, a `danger` toast, "We
...
One row per transaction, **oldest first**, in three groups of columns: **what a person writes** — the
first eleven, all a template needs; **what the app writes so an import can rebuild a download
exactly** — used when present, never required; and **what is there only to be read**, which an import
ignores.

| Column          | What it holds                                                                                                                                                                                                                                                                                    | On import                                                                                                                                                                                  |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `date`          | `2026-09-22`: the day the row froze (`dayKey`); when it has none, the day in the current time zone, as the list does                                                                                                                                                                             | Required unless `instant` is there                                                                                                                                                         |
| `time`          | `08:42`, in the current time zone; _About_ names the zone                                                                                                                                                                                                                                        | Optional, read in the importer's time zone                                                                                                                                                 |
...
| `account`       | The name of the account the row touches first: the source, or the destination when there is only one (an income, a raised adjustment, a payment that arrived); archived ones included                                                                                                            | Required unless `account_id` resolves; matched against **active** accounts                                                                                                                 |
| `to_account`    | The destination of a transfer, empty otherwise                                                                                                                                                                                                                                                   | Required for a transfer, the same way                                                                                                                                                      |
| `category`      | The category's name, archived ones included; empty when there is none                                                                                                                                                                                                                            | Optional, matched against active categories; one of another type is refused                                                                                                                |
...
| `tags`          | The tags, lower case, joined by a comma and a space: `coffee, latte`                                                                                                                                                                                                                             | Optional. A comma inside a tag splits it; _About_ says so                                                                                                                                  |
| `instant`       | The exact moment in UTC, `2026-09-22T13:42:00Z`                                                                                                                                                                                                                                                  | Preferred over `date` and `time` when present, so a row lands on the same day in any zone                                                                                                  |
| `id`            | The transaction's id                                                                                                                                                                                                                                                                             | A row whose id **is already in your ledger** is skipped, so importing a download twice duplicates nothing — an edited row too, and _About_ says so. It is never reused as the new row's id |
| `account_id`    | The id of `account`                                                                                                                                                                                                                                                                              | Preferred over the name when it is one of yours, since archived and active accounts can share a name                                                                                       |
| `to_account_id` | The id of `to_account`                                                                                                                                                                                                                                                                           | The same                                                                                                                                                                                   |
| `category_id`   | The id of `category`                                                                                                                                                                                                                                                                             | The same                                                                                                                                                                                   |
| `your_share`    | A shared expense's share of yours — the split's share, the figure the row shows — with the amount's sign; empty on every other row                                                                                                                                                               | Ignored                                                                                                                                                                                    |
| `to_review`     | `true` while it is still to review, else `false`                                                                                                                                                                                                                                                 | Ignored                                                                                                                                                                                    |
| `source`        | `MANUAL`, `QUICK` or `IMPORT`                                                                                                                                                                                                                                                                    | Ignored                                                                                                                                                                                    |
| `sync`          | `PENDING` or `ATTENTION` while it, or the shared expense it carries, is only on this device — the rule of the row's badge; empty otherwise. A row that needs attention holds the server's version, not the change the server refused, and a new row the server refused is not in the file at all | Ignored                                                                                                                                                                                    |

A name that matches only an archived account or category is refused on import, never silently moved
to another. **A download is not a backup**, and _About_ says which rows an import will not rebuild: a
shared expense comes back as a plain expense for its full amount, without its group; a line added from
a group shared with you loses that link; a payment between people is not imported.
...
as numbers with their currency's decimals, and a list to pick `type` from — the four importable types, over the whole column, so a template with its rows deleted keeps it; the sheet _About_, in the
...
description can come from somebody else's group and must never run as a formula; an import removes a `'` only when the next character is one of those, and never touches `amount`, and the Excel file needs none, because its text cells are never formulas. Summing
